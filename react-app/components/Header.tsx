...
export default function Header(props: { items: CartItem[], activeLink: SectionId, scrollToSection: ScrollToSection, toggleDrawer: ToggleDrawer, handleSearch: (arg: React.ChangeEvent<HTMLInputElement>) => void}) {
    ...
    useEffect(() => {
        if(!hideConnectBtn) setHideConnectBtn(true);
                    ...
                    <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
                        <div className="relative flex h-16 justify-between">
                            ...
                            <div className="float-right flex flex-shrink-0 items-center">
                                <h3 className={`absolute right-1 text-xs text-orange-900 font-bold ${items?.length > 0 && "font-black text-lg"}`}>{items?.length}</h3>
                                <button className={`rounded-full bg-white text-orange-900 p-3`} onClick={toggleDrawer("bottom", true, "cartContent")}>
